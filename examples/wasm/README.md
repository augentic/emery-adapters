...
- **Timeouts:** `GUEST_TIMEOUT_MS` is increased to one hour (from the 30s default) to give the model enough time to finish. Per-spawn `cursor-agent` wall-clock uses the Cursor backend default (600s); raise `CURSOR_TIMEOUT_SECS` in `.env` if a leg needs longer.
