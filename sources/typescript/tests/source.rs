//! End-to-end test of the TypeScript component over `emery:adapter/source`:
//! the built component runs through the omnia runtime, driven by the
...
// source tree for the prompt to walk; the inline leg carries the driver's
// value across the `Value` arm and lends nothing. `read_doc` answers with the
// embedded body across the tool streams, and the component's `check` accepts
// each candidate over the same streams.
#[tokio::test]
