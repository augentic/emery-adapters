...
//! (`SPECIFY_TYPESCRIPT_MCP_URL`, the guest-kit `mcp_url` convention),
...
use specify_typescript_core::{operations, registry};
...
impl Guest for TypeScriptAdapter {
    async fn survey(id: AdapterId) -> Result<Vec<Lead>, Error> {
        let url = mcp_url("typescript");
