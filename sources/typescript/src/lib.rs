...
//! `specify-typescript-core`. See `specify_guest_kit::source` for the
//! shim contract.
...
impl Guest for Adapter {
    fn describe(_id: AdapterId) -> Manifest {
        specify_typescript_core::operations::describe().into()
    }
    ...
    async fn survey(id: AdapterId) -> Result<Vec<Lead>, Error> {
        ...
        specify_typescript_core::operations::survey(&WasiModel, &ctx)
            .await
    ...
    async fn extract(id: AdapterId, lead: Lead) -> Result<Evidence, Error> {
        ...
        let url = shelf::mcp_url("typescript");
        let ctx = seam::Context::guest(&id, url.as_deref());
        specify_typescript_core::operations::extract(&WasiModel, &ctx, &lead)
            .await
            .map(Into::into)
            .map_err(Into::into)
    }
...
impl wasip3::exports::http::handler::Guest for HttpGuest {
    async fn handle(
        ...
        shelf::Shelf {
            server_name: "specify-typescript-references",
            version: env!("CARGO_PKG_VERSION"),
            docs: specify_typescript_core::registry::docs(),
        }
        .serve(request)
        .await
    }
