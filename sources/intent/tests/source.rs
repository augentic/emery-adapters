//! End-to-end test of the intent component over `emery:adapter/source`: the
//! built component runs through the omnia runtime, driven by the
//! `source_extract` program from `crates/test-programs` over the same seam
//! the engine uses, against a scripted host-side model. The driver asserts
//! what crosses the boundary (and traps on failure); this side asserts what
//! only the host sees: the completions the component opened and the
//! exchanges it drove. The adapter's own behaviour is `tests/extract.rs`'s.
...
/// The operator's brief, the one file of the lent tree.
const BRIEF: &str = "Let users reset passwords by email.";

/// An intent evidence answer the claim gate accepts.
const EVIDENCE: &str = r#"{"authority":"intent","claims":[
...
// prompt is the compiled-in `prompts/extract.md`. The workspace leg reads the
// brief through the mounted tree and puts it as the turn's material while
// lending the tree; the inline leg carries the driver's value across the
// `Value` arm and lends nothing. `read_doc` answers with the embedded body
// across the tool streams, and the component's `check` accepts each
// candidate over the same streams.
#[tokio::test]
async fn source_extract() {
    ...
    let status = Deployment::new()
        ...
        .guest(test_programs::ADAPTER, test_programs::ADAPTER_INTENT)
        .command("caller")
    ...
    assert!(
        seen[0].messages[0].contains(BRIEF),
        "the brief is the material: {:?}",
        seen[0].messages
    );
    assert!(
