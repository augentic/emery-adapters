//! End-to-end tests for the intent component over `emery:adapter/source`:
//! every scenario runs the built component through the omnia runtime, driven
//! by a guest program from `crates/test-programs` over the same seam the
//! engine uses, against a scripted host-side model. The driver asserts what
//! crosses the boundary (and traps on failure); this side asserts wire
//! fidelity: the completions the component opened and the exchanges it drove.
...
const UNSTATED: &str =
    r#"{"authority":"intent","claims":[{"kind":"requirement","id":"password-reset.request"}]}"#;

/// The one-file tree the adapter reads as the operator's brief.
fn project() -> Scratch {
    let project = scratch();
    project.write("brief.md", "Let users reset passwords by email.\n");
    project
}
