//! The intent adapter guest: `wasm32` shim over
//! `specify-intent-core`.
//!
...
//! (`SPECIFY_INTENT_MCP_URL`, the guest-kit `mcp_url` convention),
...
use specify_intent_core::{operations, registry};
...
struct IntentAdapter;

impl Guest for IntentAdapter {
    async fn survey(id: AdapterId) -> Result<Vec<Lead>, Error> {
        let url = mcp_url("intent");
