...
//! `specify-documentation-core`. See `specify_guest_kit::source` for
//! the shim contract.
...
impl Guest for Adapter {
    fn describe(_id: AdapterId) -> Manifest {
        specify_documentation_core::operations::describe().into()
    }
    ...
    async fn survey(id: AdapterId) -> Result<Vec<Lead>, Error> {
        ...
        specify_documentation_core::operations::survey(&WasiModel, &ctx)
            .await
    ...
    async fn extract(id: AdapterId, lead: Lead) -> Result<Evidence, Error> {
        ...
        let url = shelf::mcp_url("documentation");
        let ctx = seam::Context::guest(&id, url.as_deref());
        specify_documentation_core::operations::extract(&WasiModel, &ctx, &lead)
            .await
            .map(Into::into)
            .map_err(Into::into)
    }
...
impl wasip3::exports::http::handler::Guest for HttpGuest {
    async fn handle(
        ...
        shelf::Shelf {
            ...
            docs: specify_documentation_core::registry::docs(),
        }
        .serve(request)
        .await
    }
