...
Walk `$SOURCE_DIR` (a read-only preopen of the operator-bound docs path) and emit one `Lead` per document surface. The engine persists the result; this prompt returns the lead-block payload only.
