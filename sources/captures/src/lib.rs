...
//! (`SPECIFY_CAPTURES_MCP_URL`, the guest-kit `mcp_url` convention),
...
use omnia_guest::mcp;
use specify_captures_core::{operations, registry};
...
struct CapturesAdapter;

impl Guest for CapturesAdapter {
    async fn survey(id: AdapterId) -> Result<Vec<Lead>, Error> {
        let url = mcp_url("captures");
