...
impl Source for Adapter {
    ...
    async fn extract<P: Model>(
        ...
        let user = format!(
            "Extract Evidence from the runtime-capture source bound to adapter `{id}` \
             (source key `{key}`) for this lead (one captured handler):\n\n{lead}\n\n\
             {content}\n\n\
             ...
             `replay-digest` / `input` / `output` body fields the prompt describes), \
             without the envelope `lead` key — this call names the lead. The caller \
             persists the document; do not write it yourself.",
            id = ctx.adapter_id,
            key = input.key,
            lead = lead.render(),
