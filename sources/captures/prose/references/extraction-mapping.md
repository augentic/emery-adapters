...
- **Cross-source synthesis.** Do not reconcile this lead's claims with another source's Evidence — that is core synthesis's job in `/emery:refine`. Emit Evidence purely from `$SOURCE_DIR`.
