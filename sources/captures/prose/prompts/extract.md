...
This prompt runs once per `(source, lead)` pair whose adapter is `captures`. Your job: locate the matching `tests/data/replays/<handler>/` directory under `$SOURCE_DIR`, read every scenario capture, and emit one Evidence YAML document. The engine persists the result; this prompt returns the YAML body only.
...
The engine persists this document and owns its source identity; neither the source key nor the adapter is written in-document.
