...
//! (`SPECIFY_SCREENSHOTS_MCP_URL`, the guest-kit `mcp_url` convention),
...
use specify_screenshots_core::{operations, registry};
...
struct ScreenshotsAdapter;

impl Guest for ScreenshotsAdapter {
    async fn survey(id: AdapterId) -> Result<Vec<Lead>, Error> {
        let url = mcp_url("screenshots");
