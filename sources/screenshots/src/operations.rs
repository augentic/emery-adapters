...
impl Source for Adapter {
    ...
    async fn extract<P: Model>(
        ...
        let user = format!(
            "Extract Evidence from the screenshots source bound to adapter `{id}` \
             (source key `{key}`) for this lead (one screen, possibly with state and \
             platform variants):\n\n{lead}\n\n\
             {content}\n\n\
...
fn content_note(input: &SourceInput) -> String {
    match &input.content {
        SourceContent::Workspace(view) => format!(
            "`$SOURCE_DIR` is the read-only CID view at `{}` — the directory of screen \
...
fn survey_user(ctx: &Context<'_>, input: &SourceInput) -> String {
    ...
    input.focus.as_ref().map_or_else(
        || {
            format!(
                "Survey the screenshots source bound to adapter `{id}` (source key `{key}`).\n\n\
             {content}\n\n\
