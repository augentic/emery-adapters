#![cfg(target_arch = "wasm32")]

use emery_sdk::{AdapterMetadata, Context, Error, Evidence, Model, SourceKind};
use test_programs::maximal;
use tracing::Level;

emery_sdk::source_adapter!(metadata, extract);

fn metadata() -> AdapterMetadata {
    emery_sdk::metadata(SourceKind::Documentation)
}

async fn extract<P: Model>(_ctx: &Context<'_, P>) -> Result<Evidence, Error> {
    progress().await;
    detail().await;
    sdk_detail().await;
    foreign_detail().await;
    Ok(maximal())
}

#[tracing::instrument(level = Level::INFO)]
async fn progress() {}

#[tracing::instrument(level = Level::DEBUG)]
async fn detail() {}

#[tracing::instrument(level = Level::DEBUG, target = "emery_sdk")]
async fn sdk_detail() {}

#[tracing::instrument(level = Level::DEBUG, target = "serde_json")]
async fn foreign_detail() {}
