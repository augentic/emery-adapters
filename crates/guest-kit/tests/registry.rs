...
fn body_returns_guaranteed_documents() {
    assert_eq!(body(DOCS, "briefs/build.md"), "# build");
}

#[test]
#[should_panic(expected = "document `briefs/missing.md` is not embedded")]
fn body_panics_on_a_registry_miss() {
    let _ = body(DOCS, "briefs/missing.md");
}
