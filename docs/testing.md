...
Operator-invoked, never CI: the runner spawns the sibling shipped `emery` binary over the built components, drives `init` + `specify`, records typed outcomes, grades the committed spec set, and writes the dated scorecard the emery release gate verifies. How to run: [`examples/eval/README.md`](../examples/eval/README.md).
...
Current posture: every source adapter and the eval runner carry **zero** `src` unit tests — behavior lives entirely in `tests/` suites.
