...
| 2   | Seam suites        | Every `sources/*` component under the omnia runtime, driven by a guest program over `emery:adapter/source`: instantiation, effect-free `metadata`, both `SourceInput` arms and the lend, the reference-tool round-trip, WIT bindings lowering of evidence and the typed `error`; plus the embedded corpus cap of every adapter | `cargo nextest run -p <adapter> --test source`, `cargo nextest run -p emery-adapters` / `make test` |
...
- `programs/source/` — the one driver over the `emery:adapter/source` seam, `extract`, universal across adapters: `metadata` checked, then — with no argument — `extract` over the lent workspace and over an inline value, each answer checked against the contract's claim gate; or — given one argument, an Omnia error code — `extract` over the lent workspace must fail and lift to that class.
...
Host suites live in the crate under test, one flat file per seam, with no shared support module:

- `sources/<name>/tests/source.rs` — `test_programs::foreach_source!()`, so every `programs/source/*` driver must have a same-named test (`source_extract`) in every adapter. The test stages the adapter's minimal tree in an `omnia_test::host::scratch()` directory, describes the deployment through `omnia_test::host::Deployment` — the driver as the `wasi:cli/run` guest beside the built component registered as `test_programs::ADAPTER` under the `emery:adapter/source` link — scripts the host-side model (`omnia_test::host::ScriptedModel`, a `WasiModelCtx` double that can drive `read_doc` through the session before answering, swapped into `Backends::defaults()`), and asserts only what the host alone sees: `ExitStatus::SUCCESS` from the driver's own checks, the script exactly consumed, one completion per `extract` (so `metadata` made none), the compiled-in `prompts/extract.md` as the system prompt, the driver's inline value arriving in the second turn (the `Value` arm crossed intact), the lend present for the workspace leg and absent for the inline one, the `read_doc` exchange returning that same embedded body, and the `check` exchanges accepting. An adapter adds only what its component alone shows the host — intent asserts the brief it read through the mount is the first turn's material. The request shape the SDK builds (tools, `check`, the key in the turn) is the SDK's suite's, not repeated here.
- Root `tests/probe.rs` — `test_programs::foreach_probe!()`; runs the `source_extract` driver with the expected code against each probe, proving each WIT `error` arm lifts back to its Omnia class with no model turn consumed. The SDK's own refusal — a candidate its claim gate rejects until the backend's budget is spent — crosses the same arm and is the SDK suite's to prove; no adapter re-proves it under the runtime.
- Root `tests/prose.rs` — `test_programs::foreach_adapter!()`, the orphan guard for the adapter set: a new `sources/<name>` fails to compile here until a test of that name exists. Each asserts the one corpus rule nothing else enforces: the 800 non-blank-line cap. Reference presence is the embed-time walker's — a dangling link fails the build.
