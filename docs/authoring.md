...
pub static PROSE: &[Doc] = emery_sdk::prose!("../prose", [
    "prompts/extract.md",
...
/// The prompt and reference documents embedded in the adapter.
pub static PROSE: &[Doc] = emery_sdk::prose!("../prose", [/* step 2 */]);
...
- [ ] `src/lib.rs` carries no logic beyond the `pub static PROSE: &[Doc] = emery_sdk::prose!("../prose", [..]);` list and, for `wasm32` alone, `mod survey;` and the `mod guest` — `emery_sdk::source_adapter!(metadata, extract)` and the two fns it names; nothing inside `src/survey.rs` names a target or a backend.
