...
- **Answers are schema-gated and repaired.** `emery_adapter::evidence(model, ctx, system, user)` sends the prompt against the generated `Evidence` schema, runs the claim gate over the reply, and re-prompts with the findings up to `emery_adapter::MAX_REPAIRS` times before failing. `emery_adapter::content_note(input, tree)` is the shared prompt fragment describing the bound workspace or inline value.
...
- **`evidence` owns the parse-and-retry loop.** An adapter builds the prompt and nothing else; the schema, the gate, and the repair budget are the SDK's.
