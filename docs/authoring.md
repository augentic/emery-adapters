...
- [ ] The guest's `extract` is one `emery_sdk::extract(&Provider, id, input, DOCS, async |model, ctx| …)` whose closure calls `survey` — nothing else: the adapter states its seams through `survey` — mechanically, or with one model call through `survey::by_model` — and every model call goes through the SDK.
