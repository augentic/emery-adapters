...
pub fn survey(ctx: &Context<'_>) -> Result<Vec<Seam>, Error> {
    ...
    let groups = Tree::list(root, |entry| entry.extension() == Some("md"))?.by_directory(FLOOR);
    if groups.len() < 2 {
        return Ok(vec![Seam::Whole]);
    }
...
An adapter whose source is never worth splitting returns `vec![Seam::Whole]` — `intent` does, after refusing an empty brief. A survey that asks the model is `async fn survey<P: Model>(model: &P, ctx: &Context<'_>, docs: &'static [Doc])`, one fn on every target — the guest passes it the model `extract` lends and its `DOCS`; tests a scripted model and their own embed — `typescript`'s lists its production modules as above, keeps a tree no directory cut would split as `Seam::Whole` with no turn spent, and otherwise hands the listed tree to the model with `tree.by_model(model, ctx, docs, FLOOR).await?`, turning each group it returns into one `Seam::Note`; `tests/survey.rs` embeds the same tree with `emery_sdk::include_prose!("../prose")` of its own. The one thing that survey adds to the crate is its prompt, `prose/prompts/survey.md` (step 6).
...
- [ ] `src/lib.rs` carries no logic beyond the `wasm32`-only `mod guest` — its `static DOCS: &[Doc] = emery_sdk::include_prose!("../prose");`, its `Provider`, and the `Guest` impl — and `pub mod survey`; reusable logic is wasm-free library code, and none of it is `cfg`-gated.
- [ ] The guest's `extract` is one `emery_sdk::extract(&Provider, id, input, DOCS, async |model, ctx| …)` whose closure calls `survey` — nothing else: the adapter states its seams through `survey` — mechanically, or with one model call through `Tree::by_model` — and every model call goes through the SDK.
