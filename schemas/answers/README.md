...
This copy is a temporary pin alongside the `wit/` pin: once the `augentic:specify` package distribution carries the answer schemas, this directory is deleted. Until then, keep it byte-identical to upstream — `cargo make check-pins` compares against a sibling `../specify` checkout when one is present. Never edit these files here; change the canonical schema in the specify repo and re-vendor.
