...
| Runner         | [`harness/live.rs`](../live.rs) (the `live` `[[test]]` target) — one `#[ignore]`d test per scenario under `contracts::`, plus the non-ignored `contracts::wiring` smoke CI runs model-free                                                                                                                                                                  |
...
cargo test -p harness --test live -- --ignored --nocapture contracts::metadata
```
