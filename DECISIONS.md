...
> The engine repo (`augentic/specify`) owns and publishes the adapter contract as the wasm-pkg package `specify:adapter`; this repo only consumes it. The vendored copy lives at `wit/deps/specify/specify.wit` (conventional wasm-tools deps layout); the root `wit/specify.wit` copy is deleted. The pin is declared in exactly one place — the `WIT_PIN` env var at the top of `Makefile.toml` — and `cargo make wit-vendor` refreshes the vendored file from the published package (temp-file fetch, move on success, so a failed fetch never corrupts the vendored copy). `cargo make wit-vendor-sibling` is the dev-loop override: it copies `../specify/wit/specify.wit` into the vendored location while a contract change is iterating in the engine before the new version is published; the published pin is the release posture.
>
