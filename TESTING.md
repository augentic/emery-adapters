...
The root `adapters` package declares the first-party catalog (`adapters::catalog()` in `src/catalog.rs`) over the engine-owned `native` host, and the root `eval` example (`examples/eval/`) composes it. The engine's `native` crate supplies the catalog machinery, provider, reference hosting, and command execution, and its `probe` library (consumed with default-features off plus the `client` feature) supplies telemetry, deterministic grading, the trial/scenario runners, and the shared cursor composition (`probe::client`); `cargo make eval` passes the trial inputs explicitly. The example owns the Tokio runtime, the catalog binding, and the scenario root, and runs production verb handlers natively without building WebAssembly. Generic catalog, provider, reference, and command mechanics are tested in `specify/crates/native/tests` (scenario/sandbox mechanics in `specify/crates/probe/tests`); this repo's binding carries only the catalog inventory check in `tests/catalog.rs`. `cargo make ci` builds and lints the package and example like any other member; the live rungs below stay operator-invoked.
