...
- An adapter's own extract behavior — the material it puts, its fail-closed refusals, prompt landing — belongs in its `tests/extract.rs` (scripted `omnia_test::guest::Scripted`; no credentials). What every adapter shares (the request shape, the claim gate's repair and spent-rounds refusal) is the SDK's suite's, asserted once. Do not widen public APIs solely for tests.
- The component rung (inside `make test`) is the seam suites over `crates/test-programs`, whose `build.rs` runs one `omnia_test::build::Components` build into one `gen.rs` — `scan_packages("sources").group("adapter")` nested-builds every `sources/*` component to `wasm32-wasip2` as `ADAPTER_<NAME>` + `foreach_adapter!`; `scan("crates/test-programs/programs")` builds each `programs/<group>/<scenario>.rs` program as `<GROUP>_<SCENARIO>` + `foreach_<group>!`, syncing the `[[example]]` stanzas below the marker in its `Cargo.toml`. Programs are `#![cfg(target_arch = "wasm32")]` guests (`omnia_guest::command!(scenario)`) that assert and trap: `programs/source/extract.rs` is the one driver over the `emery:adapter/source` seam — `metadata`, then `extract` over the lent workspace and an inline value expecting gated evidence, or, given one argument, an Omnia error code, `extract` expecting that refusal class; `programs/probe/` are fixture adapters standing in for the one under test, one per WIT `error` arm (`refusing`, `upstream`). Host suites live in the crate under test with no shared support module: each `sources/<name>/tests/source.rs` invokes `foreach_source!()` against its own component (registered as `test_programs::ADAPTER`), the root `tests/probe.rs` invokes `foreach_probe!()` and runs the driver with the expected code against each probe, and the root `tests/prose.rs` invokes `foreach_adapter!()` over every adapter's embedded corpus — so a new program fails to compile until every adapter tests it, and a new `sources/<name>` fails to compile until the root names it. The rung owns the boundary only — instantiation, effect-free `metadata`, both `SourceInput` arms and the workspace lend, the reference-tool round-trip, WIT bindings lowering, the compiled-in prompt — never adapter behaviour (that is `tests/extract.rs`, natively, and the SDK's suite for what every adapter shares), prompt text, or quality.
- Use `cargo nextest`, not bare `cargo test`; process isolation is required by environment-mutating suites.
- The live rung is operator-invoked, never CI: a runner spawning the shipped `emery` binary over the built components, grading the committed spec via `emery show spec` into a dated scorecard. It is being recreated under `examples/` as a root example; until then there is no `make eval`.
...
cargo nextest run -p NAME # focused adapter tests (its extract and seam suites)
cargo nextest run -p emery-adapters # the root seam suites alone: probes and corpora
