...
- The component rung (inside `make test`) is the seam suites over `crates/test-programs`, whose `build.rs` runs `omnia_test::build::Components` twice — `scan_packages("sources")` nested-builds every `sources/*` component to `wasm32-wasip2` as `ADAPTER_<NAME>` + `foreach_adapter!`; `scan("crates/test-programs/programs")` builds each `programs/<group>/<scenario>.rs` program as `<GROUP>_<SCENARIO>` + `foreach_<group>!`, syncing the `[[example]]` stanzas below the marker in its `Cargo.toml`. Programs are `#![cfg(target_arch = "wasm32")]` guests (`omnia_guest::command!(scenario)`) that assert and trap: `programs/source/` drives the `emery:adapter/source` seam (`roundtrip`, `refused`); `programs/probe/` are fixture adapters standing in for the one under test, one per WIT `error` arm (`refusing`, `upstream`). Host suites live in the crate under test with a local `run_guest` and no shared support module: each `sources/<name>/tests/source.rs` invokes `foreach_source!()` against its own component (registered as `test_programs::ADAPTER`), the root `tests/probe.rs` invokes `foreach_probe!()`, and the root `tests/prose.rs` invokes `foreach_adapter!()` over every adapter's embedded corpus — so a new program fails to compile until every adapter tests it, and a new `sources/<name>` fails to compile until the root names it. The rung owns the boundary only — instantiation, effect-free `metadata`, the reference-tool round-trip, WIT bindings lowering, the compiled-in corpus — never prompt text or quality.
