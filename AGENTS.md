...
| Path | Role |
| --- | --- |
| `sources/<name>/` | One adapter: `Cargo.toml` (`cdylib` + `rlib`; `[dependencies]` is `emery-sdk` alone, `emery-prose` the `emit` build dependency); `build.rs` = `emery_prose::emit("prose")`; `src/lib.rs` = `pub const KIND`, `emery_sdk::registry!()` as `docs`, and the module declarations; `src/survey.rs` = `pub fn survey`; `src/guest.rs` = the `wasm32`-only `impl Guest`; `prose/{prompts/extract.md, references/, rules/}`; `tests/survey.rs` |
...
| `examples/` | The `emery.toml` configs the shipped `emery` binary runs over the `make release` components, and the fixtures they lend ([examples/README.md](examples/README.md)) |
