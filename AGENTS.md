...
| Path | Role |
| --- | --- |
| `sources/<name>/` | One adapter: `Cargo.toml` (`cdylib` + `rlib`; `[dependencies]` is `emery-sdk` alone, `emery-prose` the `emit` build dependency); `build.rs` = `emery_prose::emit("prose")`; `src/lib.rs` = the `wasm32`-only `mod guest { impl Guest }`, `pub mod prose { emery_sdk::include_prose!() }`, and `pub mod survey`; `src/survey.rs` = `pub fn survey`; `prose/{prompts/extract.md, references/, rules/}`; `tests/survey.rs` |
