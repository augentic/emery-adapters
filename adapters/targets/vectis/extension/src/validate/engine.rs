//! Deterministic validation engine for the `vectis validate` subcommand.
//!
//! Public surface: [`run`] dispatches a parsed [`ValidateArgs`] to the
//! per-mode handler. Each per-mode envelope carries a uniform shape:
//!
//! ```json
//! {
//!   "mode": "assets",
