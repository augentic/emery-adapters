...
2. **Mergeability.** The baseline-conflict check (`emery slice validate` review diagnostics, re-checked by the merge phase) compares the slice's `defined-at` timestamp against the baseline files it intends to replace. Edits to the baseline outside this audit trail will be flagged as conflicts at merge time and may be silently lost.
