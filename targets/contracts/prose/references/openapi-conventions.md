...
| `x-emery-id` | **Optional rename-stable identifier** (contract identity/version validation). When present, MUST match `^[a-z][a-z0-9-]*$` and be ≤ 64 characters; MUST be unique across every top-level contract under root `contracts/`. The id survives file moves and `info.version` bumps — once set on a contract, never change it. SHOULD be set on new top-level OpenAPI documents (typically the file stem, e.g. `user-api` for `user-api.yaml`). The contract's path under root `contracts/` remains canonical; the id is a hint, not a substitute. |
