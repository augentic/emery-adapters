...
**Output** — normalised OpenAPI 3.1.0 files in `$CONTRACTS_DIR/http/`, with inline schemas decomposed into `$CONTRACTS_DIR/schemas/` and Emery metadata injected. The input files are replaced in-place with their normalised equivalents; decomposed schemas are added as new files.
