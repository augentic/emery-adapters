...
//! blocking findings when on-disk bytes diverge; `vectis sync android-scaffold`
...
fn drift_message(relative_path: &str, on_disk: &str) -> String {
    let mut message = format!(
        "{relative_path} diverges from the embedded Android scaffold template; agents must not edit this file — run `vectis sync android-scaffold`"
    );
...
fn missing_scaffold_message(relative_path: &str) -> String {
    format!(
        "{relative_path} is missing; CLI-owned scaffold files must be present — run `vectis sync android-scaffold`"
    )
}

fn unreadable_scaffold_message(relative_path: &str, err: &std::io::Error) -> String {
    format!(
        "{relative_path} could not be read ({err}); CLI-owned scaffold files must match the embedded template — run `vectis sync android-scaffold`"
    )
}
