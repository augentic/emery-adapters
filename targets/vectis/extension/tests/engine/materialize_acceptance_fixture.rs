...
use specify_vectis::materialize::paths::{Platform, export_layout};
...
fn assert_layout_artifacts_exist(design_root: &Path, role: &str, kind: &str, asset_id: &str) {
    for platform in [Platform::Ios, Platform::Android] {
        ...
        for rel in &layout.artifacts {
            ...
            assert!(path.is_file(), "missing acceptance export artifact: {}", path.display());
        }
        let pin_path = design_root.join(&layout.pin);
        assert!(pin_path.exists(), "missing acceptance export pin target: {}", pin_path.display());
    }
}
...
fn acceptance_fixture_validates_cleanly() {
    ...
    let errors =
        envelope.get("errors").and_then(serde_json::Value::as_array).expect("errors array");
    assert!(errors.is_empty(), "acceptance fixture should validate cleanly: {envelope}");
}
