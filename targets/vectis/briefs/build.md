...
Each writer / reviewer phase sub-brief runs in its **own sub-agent** with a clean context window. Core test verify-repair and Android shell verify still run inside their phase sub-agents. **iOS verify is an exception:** after the `ios-writer` sub-agent returns, the orchestrator (not a verify sub-agent) runs `sync ios-scaffold`, `swiftformat`, `make build`, and `make sim-build`; failures spawn `ios-verify-repair` sub-agents that may edit Swift only. `/spec:build` coordinates the sequence and executes the iOS verify shell commands inline.

**Inputs (orchestrator → sub-agent):** `task` (one of `core-writer`, `test-writer`, `ios-writer`, `ios-verify-repair`, `android-writer`, `core-reviewer`, `ios-reviewer`, `android-reviewer`), `arguments` (standard arguments above), `mode` (`create`, `update`, or `repair` — decided by the orchestrator from on-disk inspection), `skip_verification` (true for shell writers; iOS verify commands run from the orchestrator, not a verify sub-agent), `artifact_paths` (paths to `spec.md`, `design.md`, `proposal.md`, regenerated `composition.yaml`, sibling `tokens.yaml` / `assets.yaml` when present, and `components.yaml` when `CATALOG_PATH` exists), `forbidden_paths` (`ios-verify-repair` only — `iOS/Makefile`, `iOS/project.yml`, `iOS/.vectis/sim-build.sh`), `allowed_paths` (`ios-verify-repair` only — Swift under `iOS/<APP_NAME>/`, plus `Theme/`, `Components/`, `Resources/`), `orchestrated` (reviewer sub-agents only; signals that the reviewer is running inside a build phase so its `design_findings` should flow into § Consolidate review findings — reviewers always return `design_findings` for the parent to consolidate, never auto-spawn follow-up slices), `extra_context` (phase-specific: `error_output` for `ios-verify-repair`, baseline test log for regression checks, prior phase warnings).
