...
5. (When `android` is in scope) Load [`build/android/write.md`](build/android/write.md) — generate / update the Compose shell. After the writer sub-agent returns, run `specify extension run vectis -- sync android-scaffold` once, then run the orchestrator-owned Android verify loop from that brief (the orchestrator executes sync / `make verify` — repair sub-agents are Kotlin-only).
