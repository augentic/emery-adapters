...
- **create** — the shell tree is absent on disk → the adapter stands up the minimum shell deterministically from its embedded scaffold templates before the write legs run (the scaffold prelude in each leg's prompt reports what was rendered); generate into the scaffolded tree. Standing up an absent shell is this adapter's own build-time responsibility — there is no separate plan-time bootstrap slice; `project.yaml.platforms` already declares the intent. Only `core`, `ios`, and `android` have scaffold support today; when the prelude reports it could not scaffold (no resolvable app name), stand the tree up per the write sub-brief.
...
4. **Proceed.** Continue with the numbered phases below; composition regeneration (Phase 1) treats your fresh bindings plus the existing catalog's confirmed entries as the effective component set at [`build/composition.md`](build/composition.md) step 6 and attaches `component: <slug>` directives to every group whose skeleton matches.
...
4. (When `ios` is in scope) Load [`build/ios/write.md`](build/ios/write.md) — generate / update the SwiftUI shell. The adapter re-renders the agent-immutable iOS scaffold files deterministically after the write leg; run the orchestrator-owned iOS verify loop from that brief (the orchestrator executes swiftformat / make — repair sub-agents are Swift-only).
5. (When `android` is in scope) Load [`build/android/write.md`](build/android/write.md) — generate / update the Compose shell. The adapter re-renders the agent-immutable Android scaffold files deterministically after the write leg; run the orchestrator-owned Android verify loop from that brief (the orchestrator executes `make verify` — repair sub-agents are Kotlin-only).
