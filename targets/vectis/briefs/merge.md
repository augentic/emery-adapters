...
For cap-matrix failures that look like version-pin drift (AGP / Gradle / uniffi mismatch surfaced after pins changed in this slice), the matching repair flow is a **specify-adapters** slice to edit [`extension/versions.toml`](../extension/versions.toml) and, if needed, [`extension/templates/core/`](../extension/templates/core/) (see [build.md](build.md) § Template / version-pin drift handling). Record the failure in the stop hint and surface it; the operator decides whether the next step is a pin fix in specify-adapters, a pin rollback, or a follow-up slice.
