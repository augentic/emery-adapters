...
registry is [`../manifest.yaml`](../manifest.yaml) (`assemblies.android`); `build.rs` validates that manifest and emits the embedded `registry.rs` consumed by the adapter's deterministic `scaffold` renderer (`android` assembly).
