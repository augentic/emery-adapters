...
- **Gradle wrapper is required**: The `gradlew` script must exist before any `./gradlew` command works. `vectis scaffold android` and `vectis android setup` install a vendored Gradle wrapper pinned to the adapter's `versions.toml` (`gradle = "8.13"`). Host-specific `local.properties`, Java 21 (`org.gradle.java.home`), and NDK substitution run via `make setup-host` in the Android shell Makefile.
