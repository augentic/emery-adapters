...
registry is [`../manifest.yaml`](../manifest.yaml) (`assemblies.core`); `build.rs` validates that manifest and emits the embedded `registry.rs` consumed by the adapter's deterministic `scaffold` renderer (`core` assembly).
