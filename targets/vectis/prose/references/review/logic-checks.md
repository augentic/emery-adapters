...
- Default **`code-fix`** — revert to stub-faithful **or** perform [§ Closure eligibility](../open-gap-contract.md#closure-eligibility) B′ closure in the remediation budget (close build-editable `spec.md` scenario body + design TBD + composition `# GAP`, wire only a grounded destination, no contradictory Evidence).
