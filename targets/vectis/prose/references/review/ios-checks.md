...
**Fix**: Do not patch DX files by hand-editing them. Re-copy the drifted paths from `$TEMPLATE_DIR` with identity substitution (the copy procedure in `references/template-materialize.md`) and regenerate the Xcode project (`make -C iOS generate-project` / `xcodegen`). Limit verify-repair to Swift under `iOS/<APP_NAME>/`, plus `Theme/`, `Components/`, and `Assets.xcassets/`.
