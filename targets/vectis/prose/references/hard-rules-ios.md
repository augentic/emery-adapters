...
- **BoltFFI / pin drift**: if the app fails with FFI contract or toolchain mismatches that look like pin drift, re-copy from `$TEMPLATE_DIR` — do not invent versions. Diff pin-bearing files against the template (see [`template-capabilities.md`](template-capabilities.md) § Template / version-pin drift handling).
