...
This is **template materialize** (`vectis::scaffold::materialize`) — not asset materialize (`vectis::materialize` / the prepare prelude that exports design-system assets). The build prompt's template-materialize prelude names which declared trees are absent; this reference carries the full procedure. Runs on the **host** filesystem — the guest cannot see a sibling checkout.
