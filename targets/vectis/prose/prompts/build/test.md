...
1. **Read inputs.** `${SPEC_PATH}`, `${DESIGN_PATH}`, `${APP_RS}`, and current-slice open-GAP markers (`design.md` TBD, `${SLICE_DIR}/composition.yaml` `# GAP`, `model.yaml` scenario text when present). Use spec-to-test mapping rules: one synchronous `#[test]` per scenario, named after the scenario, with a `/// Spec: <domain> > REQ-XXX > Scenario: <scenario>` traceability comment. Full mapping rules: [`test-spec-mapping.md`](../../references/test-spec-mapping.md).
2. **Map scenarios deterministically.** Each `#### Scenario:` block produces exactly one test function. The `**WHEN**` clause becomes the test setup (model state, dispatched events). The `**THEN**` clause becomes assertions over `Command` effects and `view()` output. Stable `REQ-XXX` ID + scenario title is the drift-detection key. For [open-GAP](../../references/open-gap-contract.md) scenarios (THEN withholds the outcome — `unspecified` / operator must supply… / design TBD / composition `# GAP` / missing THEN): assert stub-faithful behaviour — effect is render (or documented stub effects), page/route/tab for the unspecified dimension **unchanged**; never assert a concrete invented destination or domain mutation. Still emit one test per scenario (LOG-008) — coverage via stub asserts, not omission. The runbook’s “vague THEN → derive from ViewModel” heuristic must **not** apply to explicit unspecified/GAP withholds ([`test-runbook.md`](../../references/test-runbook.md)).
...
6. **Do not run `cargo test` in create or update mode** — orchestration owns it. In repair mode, run `cargo test` to get fresh errors and verify the fix before returning. Preserve test names, `/// Spec:` traceability comments, and assertion intent — only adjust the syntax used to express them. When a test asserts a concrete destination while open-GAP markers remain, remediate inside this core budget: prefer routing to `core-writer` to **revert** the handler to stub-faithful (or complete [B′ closure](../../references/open-gap-contract.md#closure-eligibility) when eligible) and align the test to stub asserts — do not burn shell verify on a known open-GAP contradiction.
...
| Test asserts concrete destination / nav / invented state while open-GAP markers remain for that scenario | Open-GAP inventiveness | Prefer spawn `core-writer` to revert to stub-faithful (or complete B′ closure when eligible), then `test-writer` to stub asserts — see [`open-gap-contract.md`](../../references/open-gap-contract.md). Prefer revert when closure eligibility fails. |
