...
5. Optional prior `${SLICE_DIR}/composition.yaml` from a prior `/emery:build` run on the same slice (refining iteration). When present, preserve any operator-applied `# GAP` comments and re-validate against the updated artifacts.
...
4. **Wire bindings.** For each screen entry, add:
   ...
   - `test_id` on interactive items and groups that Maestro journeys or shell accessibility tags must target — kebab-case (`splash-cta`, `task-list-row`). Codegen derives `MAESTRO_<UPPER_SNAKE>` from the value; shell writers consume `MaestroTestIds.*`.
