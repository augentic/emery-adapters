...
3. **Universal checks (lead).** Apply every `UNI-*` rule from the shared universal codex pack (resolve via `specify rules export`) with Kotlin / Android heuristics. Full library: [`review/universal-checks.md`](../../../references/review/universal-checks.md).
