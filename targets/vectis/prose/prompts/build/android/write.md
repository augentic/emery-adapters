...
5. **Refresh generated UI surfaces.** Regenerate shell-local theme code under `…/ui/theme/` for `ANDROID_PACKAGE` (Material 3 fallback when `tokens.yaml` is absent — full templates: [`android/token-templates.md`](../../../references/android/token-templates.md)), and drawable / mipmap resources under `Android/app/src/main/res/`. Design-system integration depth: [`android/design-system-integration.md`](../../../references/android/design-system-integration.md).
   - **Canonical UI bindings.** Read [`canonical-ui-bindings.md`](../../../references/canonical-ui-bindings.md). Add `test_id: <kebab-case>` on composition items during the composition leg; add strings/errors in `${PROJECT_DIR}/contract/{ui-strings,ui-errors}.yaml`. Never hand-write `Text("…")` for product copy or test tags when a generated constant exists. Run **`cargo make generate-bindings`** after shell/core edits; build verify enforces bindings. Use **`MaestroTestIds.*`**, **`UiStrings.*`**, and **`crate::ui_strings::*`** / **`crate::ui_errors::*`** in shell and core — never duplicate values inline. Keep **`testTagsAsResourceId = true`** on the root `Surface` in `ContentView` so Maestro `id:` selectors resolve. **Save FABs:** icon-only checkmark (no visible `"Save"`); a11y via `SAVE_LIST` / `SAVE_TASK` in `ui-strings.yaml` — tap by `test_id` in Maestro, not visible Save text.
...
8. **Maestro journeys (when in scope).** If `design.md` or `tasks.md` calls for UI journey coverage, add keys to `contract/*.yaml`, run `cargo make generate-bindings`, wire test tags into composables, and author `.maestro/entries/` + `.maestro/journeys/` per [`maestro/journey-authoring.md`](../../../references/maestro/journey-authoring.md). Do **not** run `maestro test` inside the build verify loop — that runs post-drain via `cargo make maestro-android`.
