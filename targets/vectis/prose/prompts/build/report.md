...
**Deferred requirements are out of scope.** The build request's `deferred[]` set (RFC-86a D4) excluded those requirements from the build's obligations: the produced trees must carry no implementation, scaffolding, placeholders, or TODO markers for them, and the report must never claim them in its coverage declaration (`covered[]`, where the report shape carries one) — the engine's report gate rejects a report that claims coverage of a deferred requirement (`target-build-deferred-covered`). The baseline spec carries the debt; product code carries nothing.
