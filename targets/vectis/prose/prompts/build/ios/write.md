...
7. **Maestro journeys (when in scope).** When `design.md` / `tasks.md` calls for journey coverage, author `.maestro/` per [`maestro/journey-authoring.md`](../../../references/maestro/journey-authoring.md). Do **not** run `maestro test` in the build verify loop — it runs post-drain via `cargo make maestro-ios`.
