...
5. **Enforce shell boundaries.** Keep all business logic in the Rust core; the shell only renders views and performs platform I/O. Remove any `import VectisDesign` — there is no shared Swift Package; the writer emits shell-local theme + asset code exclusively.
   - **Canonical UI bindings.** Read [`canonical-ui-bindings.md`](../../../references/canonical-ui-bindings.md). Add `test_id: <kebab-case>` on composition items during the composition leg; add strings/errors in `${PROJECT_DIR}/contract/{ui-strings,ui-errors}.yaml`. Never hand-write display copy or accessibility test tags when a generated constant exists. Run **`cargo make generate-bindings`** after shell/core edits; build verify enforces bindings. Use **`MaestroTestIds.*`**, **`UiStrings.*`**, and **`crate::ui_strings::*`** / **`crate::ui_errors::*`** in shell and core — never duplicate values inline. **Save FABs:** icon-only checkmark (no visible `"Save"`); a11y via `SAVE_LIST` / `SAVE_TASK` in `ui-strings.yaml` — tap by `test_id` in Maestro, not visible Save text.
...
7. **Maestro journeys (when in scope).** If `design.md` or `tasks.md` calls for UI journey coverage, add keys to `contract/*.yaml`, run `cargo make generate-bindings`, wire test tags into views, and author `.maestro/entries/` + `.maestro/journeys/` per [`maestro/journey-authoring.md`](../../../references/maestro/journey-authoring.md). Do **not** run `maestro test` inside the build verify loop — that runs post-drain via `cargo make maestro-ios`.
