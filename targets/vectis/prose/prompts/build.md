...
The adapter core inlines this document into the system prompt of every build leg for a `target: vectis` slice; the core — not this document — sequences the legs. The build is the **generation** operation of the engine's six-operation target contract (`guidance` / `build` / `verify` / `repair` / `review` / `merge`): it produces a buildable cross-platform application (Crux shared core + per-platform shells) from the slice's already-synthesised `spec.md` and `design.md`, then answers with its own typed phase report. Verification, findings-directed repair, and standards review are **separate engine-dispatched operations** — no build leg runs a verify-repair loop, spawns a reviewer team, or writes a verify stamp. This document pins down three responsibilities in one place:
...
A private workspace is lent to the build; nothing is written to the operator's checkout directly.
...
The build runs against the build request the engine assembled; consume the inputs manifest the user prompt renders rather than relying on convention. Slice artifacts read from `STAGE_DIR` (the engine restages them before the build).

- `proposal.md` — `## Platforms` scope (`core` / `ios` / `android`) and screen / interaction intent.
- `specs/<domain>/spec.md` — behavioural requirements per domain: screen titles, scenarios, platform-specific behaviour, validation rules.
- `design.md` — domain model: ViewModel / `Event` / `Route` variants, per-page view structs, capability matrix.
- `tasks.md` — phase-completion tracking (checkboxes marked in the stage at the report leg).
- The three design-system inputs the adapter's `metadata` record declares, **all optional** (`required: false`), each with an explicit absent-fallback:
...
- On template / toolchain failure: **stop** and report the unresolved prerequisite as a blocking finding — see [Consumer tooling boundary](../references/emery-runtime/guardrails.md#consumer-tooling-boundary).
...
| `STAGE_DIR` | The writable artifact stage root named in the user prompt — the candidate slice tree. Every slice-artifact read and write goes here. |
| `SLICE_DIR` | Alias for `STAGE_DIR` wherever an older phase prompt still says `SLICE_DIR`. |
| `DOMAIN_NAME` | The single domain spec folder under `STAGE_DIR/specs/`. When the slice carries multiple domains, iterate the per-domain phase prompts in declaration order. |
| `PROJECT_DIR` | The lent workspace root — the target product tree. |
...
The adapter core processes platforms in dependency order: `core` first (the shells depend on it), then the declared `ios` / `android` shell write legs.

## Leg order

Leg order is owned by the adapter core, not by this document: the core runs its deterministic bootstrap gate (side-effect-free — a blocked build has written nothing), then the prepare/materialize prelude, then the **composition** leg (Step 0.5 + Phase 1, gated once by the in-guest composition validator — blocking findings end the build and ride the phase report; the engine owns repair routing), the **core** leg (Phases 2–3: core write + Crux test authoring, generation only), one **shell** write leg per declared shell platform (Phases 4–5, generation only), and finally the **report** leg (the phase report answer). Each leg's system prompt carries this document plus the leg's phase prompt: [`build/composition.md`](build/composition.md) (Step 0.5 + Phase 1), [`build/core/write.md`](build/core/write.md) + [`build/test.md`](build/test.md) (Phases 2–3), [`build/ios/write.md`](build/ios/write.md) / [`build/android/write.md`](build/android/write.md) (Phases 4–5, when in scope), and [`build/report.md`](build/report.md) (the phase outcome contract and the phase-report shape).

**Step 0.5 — component inference** runs in the composition leg, ahead of composition regeneration: the adapter injects the in-guest deterministic cluster report; the leg names unbound clusters by judgement and writes the bindings file to `STAGE_DIR/build/component-bindings.yaml`. The full contract lives in [`build/composition.md`](build/composition.md) § Step 0.5.

The mechanical checks (`cargo fmt` / `check` / `clippy` / `test`, per-shell `make build`), the verify stamps, findings-directed repair, and the engineering-standards reviewer teams belong to the engine-dispatched `verify`, `repair`, and `review` operations — never run them from a build leg.

## § Sub-agent delegation

Each writer phase prompt runs in its **own sub-agent** with a clean context window. Before any fan-out, fetch the full delegation contract — sub-agent task names, inputs/outputs, allowed/forbidden path scopes — from [`sub-agent-contract.md`](../references/sub-agent-contract.md) via the granted MCP references.
