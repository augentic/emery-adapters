...
/// One static asset reference discovered in a composition document.
#[derive(Debug)]
pub struct AssetRef {
    /// The referenced asset id (the `assets.yaml` key).
    pub id: String,
    /// JSON Pointer of the referencing key inside the composition.
    pub path: String,
}
