...
pub struct AutoPin {
    /// The `assets.yaml` key of the materialized asset.
    pub asset_id: String,
    /// Platform slot (`ios` / `android`) the pin fills.
    pub platform: String,
    /// Canonical pin path relative to the design-system root.
    pub path: String,
}
...
mod tests {
    ...
    // Artifact entry wins over sidecar Contents.json; auto-pins fill
    // absent slots only.
    #[test]
    fn pins_matrix() {
