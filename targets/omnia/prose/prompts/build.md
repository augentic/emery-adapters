...
> The omnia adapter core inlines this document into the system prompt of every build leg — preparation, generation, capture replay, and close-out — alongside the leg's own prompt under [`build/`](build/). Leg sequencing lives in the adapter core (`src/operations.rs`), not here: each leg's user prompt names the sections of this document to follow. Build is **generation only** (RFC-90): verification, repair, and standards review are sibling operations with their own prompts ([`verify.md`](verify.md), [`repair.md`](repair.md), [`review.md`](review.md)), dispatched by the engine under its own budgets — no build leg runs the check suite, repairs failures, or reviews standards. Synthesis idioms (provider DI, WASM guardrails, error variants, validation placement) live in [`guidance.md`](guidance.md) and must already be reflected in the slice's `specs/<domain>/spec.md` + `design.md` before the build runs.
...
The adapter core drives the legs in a fixed order — preparation ([`build/prepare.md`](build/prepare.md)), generation (crate writer, test writer, guest writer in create mode — code writing only), capture replay ([`build/replay.md`](build/replay.md), dispatched only when the build context binds a `captures` source — skipped in-guest otherwise), then close-out ([`build/report.md`](build/report.md)), which answers the build's phase report: staged `tasks.md` checkbox marking, output declaration, and the findings synthesis for the generation pass. Within the generation leg, write the crate before the tests and never transition the slice lifecycle — the engine owns the build loop, its gates, and the `Refined → Built` transition.

No leg runs the check suite (`cargo fmt` / `check` / `clippy` / `test`) or loops on failures: the engine dispatches the [`verify`](verify.md) operation after the build and routes its blocking findings through [`repair`](repair.md) under an engine-owned budget.

## § Stop hint contract

A build that cannot produce its candidate surfaces a stop hint as the body's final output — a single structured message the parent loop can act on without re-deriving context:
...
Render the hint as the final visible output of the run, alongside the blocking findings that make the build phase report fail (see the report prompt's contract in [`build/report.md`](build/report.md)). Never write the lifecycle yourself — the engine gates the phase report and owns the lifecycle, so the slice stays `refined` and the loop (or a re-invocation) re-enters cleanly.
...
- [`guidance.md`](guidance.md), [`verify.md`](verify.md), [`repair.md`](repair.md), [`review.md`](review.md), [`merge.md`](merge.md) — sibling operation prompts; [`build/prepare.md`](build/prepare.md), [`build/crate.md`](build/crate.md), [`build/test.md`](build/test.md), [`build/guest.md`](build/guest.md), [`build/replay.md`](build/replay.md), [`build/report.md`](build/report.md) — per-leg prompts.
