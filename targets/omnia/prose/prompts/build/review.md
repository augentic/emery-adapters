...
2. **Declare outputs.** List the build outputs in the answer's `outputs[]`: the slice's crate tree (`$CRATE_PATH`) and, when this build wrote the guest scaffolding (create mode), the workspace-root guest files — each as `platform: core` with a path relative to the workspace root. Declare only paths the workspace actually contains; the deterministic report gate fails the build on a declared-but-missing path.
3. **Respect the deferred set.** The build request's `deferred[]` requirements (RFC-86a D4) are out of scope end-to-end: confirm the workspace carries no implementation, scaffolding, placeholders, or TODO markers for them, and never claim them in the report's coverage declaration (`covered[]`, where the report shape carries one) — the engine's report gate rejects a report that claims coverage of a deferred requirement (`target-build-deferred-covered`).
4. **Synthesise findings.** Fold the findings left unresolved after remediation — from `REVIEW.md`, the verify-repair output, and capture replay's classification (its outcome rides the user prompt's `Phase outcomes` block) — into the answer's `findings[]` (`title` / `severity` / `impact` / `remediation`, plus `rule-id` for codex citations). Status is derived, never judged: any `critical` / `important` finding makes the assembled report `status: failure`, so a build that cannot succeed (an exhausted verify-repair budget, unclearable blocking findings, confirmed replay failures) must carry at least one blocking finding. A clean build answers with non-blocking findings only (or none).
