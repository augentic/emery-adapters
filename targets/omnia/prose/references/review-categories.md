...
- [`adapters/shared/prose/rules/universal/`](../../../shared/prose/rules/universal/) — shared `UNI-001` through `UNI-021` rules. Read these codex files directly.
...
After all three specialists report, the lead applies every `UNI-*` rule from [`adapters/shared/prose/rules/universal/`](../../../shared/prose/rules/universal/) with Omnia/WASM-specific detection. Read the first-party codex files directly. Several universal checks overlap with categories already assigned to the specialists. Skip those and focus on the gaps:
