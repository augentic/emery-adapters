...
//! at build time), and generates the base-repo scaffold template
//! registry from `templates/manifest.yaml`.
...
const EXPECTED_COUNTS: &[(&str, usize)] = &[("core", 18)];
...
fn main() {
    ...
    if let Err(err) = generate_template_registry() {
        panic!("omnia template manifest codegen failed: {err}");
    }
}
...
fn generate_template_registry() -> Result<(), String> {
    ...
    // Build inputs are co-located inside this crate: the `templates/`
    // tree carries the scaffold sources plus its `manifest.yaml` and
    // `manifest.schema.json`, so the codegen anchor is the crate dir.
    let templates_root = manifest_dir.join("templates");
