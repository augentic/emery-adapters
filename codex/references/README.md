...
`runtime/` is the single **emery-runtime bundle**: the canonical copy of the runtime references each source adapter ships. Each adapter exposes it as `references/emery-runtime/` via a single directory symlink (`sources/<name>/prose/references/emery-runtime -> ../../../../codex/references/runtime`), so adapter prompts can link with `../references/emery-runtime/...` without escaping the adapter tree. Each adapter lists the bundle's files by that path and embeds them through the symlink at compile time, so consumers receive self-contained regular files.

This README sits beside the bundle, not in it: it is contributor guidance, which the embedded corpus never carries, and the symlink exposes `runtime/` alone.
