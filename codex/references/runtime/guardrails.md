...
- **Never hand-edit `plan.yaml`.** Append entries through `emery plan add`; Gate 1 is stamped by the first `emery plan execute` (invoking execute is the approval act); walk entries backwards through `emery plan undo`; close out the plan through `emery plan archive`. The single-writer contract lives in the [`plan` skill](https://emery.augentic.io/reference/change-skills/plan.html).
