...
Standards enforcement is **not** a workflow phase: findings may block a pipeline but never transition a slice or write lifecycle fields. Plan **Gate 1** (the first `emery plan execute`) is operator approval of a plan, not engineering-standards enforcement.
