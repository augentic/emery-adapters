...
1. Clone this repository. The engine crates (`emery-adapter`, `emery-prose`) resolve as dependencies on [`augentic/emery`](https://github.com/augentic/emery), pinned by release tag (`tag = "vX.Y.Z"`) and the committed `Cargo.lock` — no sibling checkout is needed to build or test. A sibling checkout at `../emery` is required only for [co-development against uncommitted engine changes](#engine-pin-and-sibling-co-development) and for the live eval (it drives that repo's built `emery` binary).
...
- **Patch-resolution errors after editing the root `Cargo.toml`** — the path patches in `[patch.crates-io]` only resolve when `../emery` exists; pin to a release tag if you are not co-developing.
...
For sibling co-development against uncommitted engine changes, use the path patches in the root `Cargo.toml` `[patch.crates-io]` block (they point at `../emery`); they must never be active at publish time.
