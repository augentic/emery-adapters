...
- **References are cited via relative markdown links, never inlined** — the model reads a reference through `read_doc`, which answers from the adapter's `PROSE` alone, so every relative link must name a document listed there, and every listed document must be reached from a prompt by such a link (the root `tests/prose.rs` holds the list to the `prose/` tree, symlinks included, and refuses a link to a directory, an unlisted file, or a path outside the tree, and a listed document no prompt reaches).
